...
if selected_driver != "請選擇填報人":
    ...
    customer_count = st.number_input("🏠 配送點數 (今日點位數)", value=None, placeholder="輸入點數", step=1)
    
    st.divider()
    m_start = st.number_input("📈 里程(起)", value=None, placeholder="起點里程", step=1)
    p_sent = st.number_input("🚚 送板數", value=None, placeholder="送出數量", step=1)
    p_recv = st.number_input("📥 收板數", value=None, placeholder="收回數量", step=1)
...
# --- 4. 統計分析區 (對標 Excel 實測邏輯) ---
st.divider()
if st.button("📊 查看績效效益分析 (對標 Excel)"):
    with st.spinner('依據上傳之 Excel 邏輯分析中...'):
        try:
            ...
            if not df.empty:
                ...
                if not month_data.empty:
                    ...
                    for k, v in map_cols.items():
                        col_key = next((c for c in month_data.columns if v in c or k in c), None)
                    ...
                    # 績效生產力公式：分子(板數) / 分母(平均里程 + 平均點數 * 加權)
                    # 依據 Excel 模擬：指標值 = (總板數 * 10) / (平均里程 * 0.5 + 平均點數 * 2)
                    def calc_excel_score(row):
                        denominator = (row['平均里程'] * 0.5) + (row['平均點數'] * 2)
                        if denominator == 0: return 0
                        return round((row['總板數'] * 10) / denominator, 1)

                    analysis['生產力指標'] = analysis.apply(calc_excel_score, axis=1)
                    analysis['績效排名'] = analysis['生產力指標'].rank(ascending=False, method='min').astype(int)

                    st.write("🛣️ 路線生產力排名 (對標上傳 Excel 邏輯)：")
                    view = analysis[['績效排名', '路線別', '趟次', '平均里程', '平均點數', '總板數', '生產力指標']]
                    st.dataframe(view.sort_values('績效排名'), use_container_width=True, hide_index=True)
                    
                    # 獎金合計 (同步 2026-01-21 公式)
